---
title: Effect of the 2021 Virginia Minimum Wage Hike on the Employment Level of Food
  Service Workers
author: "By Jacob Langley"
date: "`r Sys.Date()`"
output:
...
```{r dataset creation, echo=FALSE, message=FALSE, include=FALSE}
...
#Employment data
va_2019_q1 <- subset(Data.clean, !is.na(av_qtr_employment) & YEAR == 2019 & STNAME == "Virginia")
va_2021_q4 <- subset(Data.clean, !is.na(av_qtr_employment) & YEAR == 2021 & STNAME == "Virginia")
va_2022_q1 <- subset(Data22.clean, !is.na(av_qtr_employment) & YEAR == 2022 & STNAME == "Virginia")
nc_2019_q1 <- subset(Data.clean, !is.na(av_qtr_employment) & YEAR == 2019 & STNAME == "North Carolina")
nc_2021_q4 <- subset(Data.clean, !is.na(av_qtr_employment) & YEAR == 2021 & STNAME == "North Carolina")
nc_2022_q1 <- subset(Data22.clean, !is.na(av_qtr_employment) & YEAR == 2022 & STNAME == "North Carolina")

tab_emp <- data.frame(
  Measure = c("VA 2019 Q1", "VA 2021 Q4", "VA 2022 Q1", "NC 2019 Q1", "NC 2021 Q4", "NC 2022 Q1"),
  Count = c(n_distinct(va_2019_q1$FIPS), n_distinct(va_2021_q4$FIPS), n_distinct(va_2022_q1$FIPS), 
            n_distinct(nc_2019_q1$FIPS), n_distinct(nc_2021_q4$FIPS), n_distinct(nc_2022_q1$FIPS)),
  Mean = c(mean(va_2019_q1$av_qtr_employment, na.rm = T), mean(va_2021_q4$av_qtr_employment, na.rm = T), 
           mean(va_2022_q1$av_qtr_employment, na.rm = T), mean(nc_2019_q1$av_qtr_employment, na.rm = T), 
           mean(nc_2021_q4$av_qtr_employment, na.rm = T), mean(nc_2022_q1$av_qtr_employment)),
  Std.Dev = c(sd(va_2019_q1$av_qtr_employment, na.rm = T), sd(va_2021_q4$av_qtr_employment, na.rm = T), 
              sd(va_2022_q1$av_qtr_employment, na.rm = T), sd(nc_2019_q1$av_qtr_employment, na.rm = T), 
              sd(nc_2021_q4$av_qtr_employment, na.rm = T), sd(nc_2022_q1$av_qtr_employment)),
  Min = c(min(va_2019_q1$av_qtr_employment, na.rm = T), min(va_2021_q4$av_qtr_employment, na.rm = T), 
          min(va_2022_q1$av_qtr_employment, na.rm = T), min(nc_2019_q1$av_qtr_employment, na.rm = T), 
          min(nc_2021_q4$av_qtr_employment, na.rm = T), min(nc_2022_q1$av_qtr_employment)),
  Max = c(max(va_2019_q1$av_qtr_employment, na.rm = T), max(va_2021_q4$av_qtr_employment, na.rm = T), 
          max(va_2022_q1$av_qtr_employment, na.rm = T), max(nc_2019_q1$av_qtr_employment, na.rm = T), 
          max(nc_2021_q4$av_qtr_employment, na.rm = T), max(nc_2022_q1$av_qtr_employment))
)

#2019 demographic data 
data_2019 <- subset(Data.clean, YEAR == 2019)

tab_2019 <- data.frame(
  Measure = c("Average age (grouped)", "Gender (1 = 100% women)", "Percent White", "Percent Black", "Percent Hisp/Latino", "Population Density"),
  
  Mean  = c(mean(data_2019$AAGE, na.rm = T), mean(data_2019$GENDER, na.rm = T), mean(data_2019$PWHITE, na.rm = T), mean(data_2019$PBLACK, na.rm = T), mean(data_2019$PHISP, na.rm = T), mean(data_2019$pop_density, na.rm = T)),
  
  Standard_Deviation = c(sd(data_2019$AAGE, na.rm = T), sd(data_2019$GENDER, na.rm = T), sd(data_2019$PWHITE, na.rm = T), sd(data_2019$PBLACK, na.rm = T), sd(data_2019$PHISP, na.rm = T), sd(data_2019$pop_density, na.rm = T)),
  
  Min = c(min(data_2019$AAGE, na.rm = T), min(data_2019$GENDER, na.rm = T), min(data_2019$PWHITE, na.rm = T), min(data_2019$PBLACK, na.rm = T), min(data_2019$PHISP, na.rm = T), min(data_2019$pop_density, na.rm = T)),
  
  Max = c(max(data_2019$AAGE, na.rm = T), max(data_2019$GENDER, na.rm = T), max(data_2019$PWHITE, na.rm = T), max(data_2019$PBLACK, na.rm = T), max(data_2019$PHISP, na.rm = T), max(data_2019$pop_density, na.rm = T)))

#2021 demo data

data_2021 <- subset(Data.clean, YEAR == 2021)

tab_2021 <- data.frame(
Measure = c("Average age (grouped)", "Gender (1 = 100% women)", "Percent White", "Percent Black", "Percent Hisp/Latino", "Population Density"),

Mean = c(mean(data_2021$AAGE, na.rm = T), mean(data_2021$GENDER, na.rm = T), mean(data_2021$PWHITE, na.rm = T), mean(data_2021$PBLACK, na.rm = T), mean(data_2021$PHISP, na.rm = T), mean(data_2021$pop_density, na.rm = T)),

Standard_Deviation = c(sd(data_2021$AAGE, na.rm = T), sd(data_2021$GENDER, na.rm = T), sd(data_2021$PWHITE, na.rm = T), sd(data_2021$PBLACK, na.rm = T), sd(data_2021$PHISP, na.rm = T), sd(data_2021$pop_density, na.rm = T)),

Min = c(min(data_2021$AAGE, na.rm = T), min(data_2021$GENDER, na.rm = T), min(data_2021$PWHITE, na.rm = T), min(data_2021$PBLACK, na.rm = T), min(data_2021$PHISP, na.rm = T), min(data_2021$pop_density, na.rm = T)),

Max = c(max(data_2021$AAGE, na.rm = T), max(data_2021$GENDER, na.rm = T), max(data_2021$PWHITE, na.rm = T), max(data_2021$PBLACK, na.rm = T), max(data_2021$PHISP, na.rm = T), max(data_2021$pop_density, na.rm = T)))


# Education
tab_Edu <- data.frame(
  Measure = c("Graduation Rate"),
  
  Mean  = c(mean(Data.clean$uni_gradrt, na.rm = T)),
  
  Standard_Deviation = c(sd(Data.clean$uni_gradrt, na.rm = T)),
  
  Min = c(min(Data.clean$uni_gradrt, na.rm = T)),
  
  Max = c(max(Data.clean$uni_gradrt, na.rm = T)))
...
# summary(lm(av_qtr_employment ~ Time + Control + Treatment, data = Data.observed))
# summary(lm(av_qtr_employment ~ Time + Control + Treatment + uni_gradrt, data = Data.observed))
...
- On May 1st 2021, Virginia raised its minimum wage from the federal minimum of $7.25 to $9.50 and again to $11.00 on January 1st 2022

- In the same timeframe Virginia’s neighbor, North Carolina, kept the federal minimum of $7.25

- This created a "natural experiment" to compare Virginia with its neighbor to see if the hike had an effect on the state’s employment level

## Abstract

- I use a Difference In Differences (DID) regression with North Carolina as a control, and logistic and lasso regressions for variable selection guidance

- The final model shows that neither minimum wage hike has had a statistically significant effect on the employment level among food and beverage workers in Virginia thus far
...
- According to a report by The Congressional Budget Office in 2019 an increase to a $15 minimum wage would raise 1.3 million people out of poverty but with a two thirds chance employment would decrease by anywhere between 0 and 3.7 million workers

- 3.7 million would be about a 65% increase in the unemployment rate

- This potential employment loss statistic is so broad it conveys very little insight into whether or not raising the federal minimum wage is dangerous for the economy

## Research Context

- It is vital that economics as a field is able to give an answer when someone asks to what degree minimum wage hikes effect employment

- The purpose of this project is to identify a specific example of a recent minimum wage change and give a more narrow response on its effect on the employment level
...
"Minimum Wages and Employment: A Case Study of the Fast-Food Industry in New Jersey and Pennsylvania” by David Card and Alan Krueger in 1994

- Examined New Jersey’s minimum wage hike in 1992 from $4.25 to $5.05 and its effect on the employment level among fast food workers in the area

- The paper used a DID method to compare the employment level on either side of the NJ/PA border before and after the change

## Research Context

### What is DID Regression?
- DID regression is a method that compares a control group and a subject group before and after a treatment is applied to the subject

- Compares differences over time and differences between the groups

- If there is a significant difference between the subject and control group after the treatment, it suggests the treatment had an impact

- The null hypothesis is that the treatment had no effect

- Utilizes cross-sectional data to mimic a traditional randomized control trials for "natural experiments"
...
- Taking inspiration from Card and Krueger, I have also utilized a DID analysis for this project

- My observations are on the county level where Card and Krueger's observations were individual restaurants 

- Like Card and Krueger, I used a neighbor state as the control

- DID as a method came under scrutiny in 2019 from a paper published in The Review of Economics and Statistics
...
### Initial Regression
- The initial regression includes all available demographic information in my created dataset
- The major question at this point is how good of a control North Carolina is for Virginia in each of these variables
- At this point we do not know which variables contain useful information and which are just measuring differences between the states


<!-- A potential issue with the original DID model is that it is unclear how good of a control North Carolina is for Virginia. If they are very dissimilar then the predictors could just be alluding to the differences in employment between the two states. If that is the case then those variables would just be doing the job of the Control variable and would be redundant and unduly pulling significance away from the treatment variable.  -->

## Methodology

### Variable Selection

- Treatment (Minimum Wage Hike) is similar enough to control that if there are proxies taking significance away from Control, they would also be shifting significance away from the Minimum Wage Hike
- The primary focus during the variable selection is to identify which variables are good predictors of employment and not just proxies for capturing the inherent differences between the two states
- Those variables that do a good job at categorizing the two states and a poor job of predicting the employment level can likely be dropped from the model

## Methodology

### Variable Selection

- The secondary goal of the variable selection is to identify whether Percent White or Percent Black is more relevant to the model since they cannot both be included because of their high inverse correlation (-96%)
...
- Logistic regression is utilized to identify which variables are good at discriminating between the two states
- If a variable is very effective at distinguishing between the two states it may not be as useful of a predictor of employment in the final model

## Methodology

### Variable Selection: Lasso Regression
- Lasso regression adds a punishment term and forces weaker predictors of the dependent variable to zero
- As the power of the punishment term increases the less useful predictors get pushed to zero first
- This method is especially useful when working with small datasets because it is less sensitive to outliers
- Between the Logistic and Lasso Regression we should get a good idea of which variables should be kept in the model for a stronger case
...
#           add.lines = list(c("\\tiny")),
#           title = "Original OLS Estimates of the Effect of Minimum Wage on the Employment Level in VA", header=FALSE, covariate.labels = c("Time", "Control", "Minimum Wage Hike", "Average age (grouped)", "Gender", "Percent White", "Percent Black",  "Percent Hisp/Latino", "Population Density", "College Graduation Rate"), font.size  = "tiny", dep.var.labels = "Employment among Food and Beverage Retailers in VA and NC", column.labels = c("2019-2021", "2019-2021", "2019-2022", "2019-2022"), star.cutoffs = c(.1,.05,.01,.001), notes = "* p<0.1; ** p<0.05; *** p<0.01; ****p<0.001", notes.append = FALSE)
...
- The DID Treatment Variable "Minimum Wage Hike" was found to be not significant

- The Control variable was found to be significant at the .05 threshold implying some of the employment level can be explained through the difference between the states
...
- Table 5 shows that average age, percent black, and percent white were not significant to the categorization 
- College Graduation Rate was significant at the p<.05 level and the rest were highly significant
- The Logistic regression had a 23% error rate in its prediction for all four versions.
...
```{r logistic, echo=FALSE, fig.height=2, fig.width=2, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
```{r lambdas, echo=FALSE, fig.height=3, fig.width=3, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}

Names <- c("Treatment", "Gender", "Time", "Pop_Density", "Perc_Black", "Perc_White", "Perc_Hisp", "Control", "Ave_Age", "Uni_Grad_Rt")

options.clear <- options.sorted

rownames(options.clear) <- Names

colnames(options.clear) <- c("log(lambda)")

grid.newpage()
grid.table(round(options.clear, 2), theme = ttheme_minimal())
...
```{r lasso, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
lbs_fun <- function(lasso.fit, ...) {
  ...
  labs.2 <- names(y.2)
  legend('left', legend=labs.2, col=1:6, lty=1, cex = 1)
}
...
```{r zoomed_w, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
```{r zoomed_b, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
- These results suggest that Percentage Black is a weaker predictor than Percentage White of the employment level among food and beverage retailers

- This creates a natural lambda selection at log(lambda) = 5.27 meaning we would drop every variable left of the double bar

- This solves our multicolinearity violation between Percent White and Percent Black

- Out of the other four variables to the left of the selected lambda; Treatment, Gender, Time, and Population Density, only Population Density was significant in the original model


## Variable Selection
### Final Model

- The DID variables will need to stay to maintain the assumptions of DID regression
- Variable Selection has allowed us to drop
  * Gender
  * Percent Hisp/Latino
  * Population Density
  * Percent Black
...
- Grouped Average Age: for every 5 years the average age of the population is older, the employment level among food and beverage retailers decreases by 471 positions, working out to be 94 positions per average year older.

- Percent White: for each percentage point higher the employment among food and beverage retailers level decreases by 10 positions
...
- Virginia raised the minimum wage again to $12 on January 1 of this year and are planning two more: $13.50 in 2025 and $15 in 2026

- A future study could collect the missing data manually and do another analysis since it appears there will be ample opportunity
...
Difference-in-difference estimation. publichealth.columbia.edu. (n.d.). Retrieved January 16, 2023, from https://www.publichealth.columbia.edu/research/population-health-methods/difference-difference-estimation

Doosa, G. (2021, June 14). The mathematical background of lasso and Ridge regression. Medium. Retrieved January 16, 2023, from https://medium.com/codex/mathematical-background-of-lasso-and-ridge-regression-23b74737c817#:~:text=Lasso%20or%20L1%20Regression%3A,directly%20performs%20feature%20selection%20also. 
...
Per the results we would expect to see that Average Age, Gender, Percent White, and Percent Black are all very similar between the two states. Whereas Percent Hisp/Latino, Population Density, College Graduation Rate, and the Employment Level are different. 


## Age

```{r age, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r gender, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r w, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r b, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r Hisp, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r pop, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r education, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r emp, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
- The DAG contains my research assumptions for the major roots of the employment level in a typical western capitalist economy.
- Grey are roots whose data is not available to me or are too abstract to include in a model
- Blue are roots that I had data for and are accounted for in the model 
...
## Research Design
### Model Design
- In order to run an appropriate regression all direct effects need to be controlled for if possible
- This means education needs to be controlled for
- It also means demographics needs to be included since it is a confounder through demand
- In this way demographics will act as a proxy for demand
...
### Criticisms of Difference in Differences

- A 2019 paper by Bruno Ferman and Cristine Pinto was published criticizing DID as a method

- They challenged the prevailing assumption that having a large number of observations for each group satisfies the homoskedasticity requirement for multilinear regression

- They also suggested  that if the treated group is much larger than the control, then DID would tend to under-reject the null hypothesis

## Research Context

### Criticisms of Difference in Differences
- In both my paper and the Card-Krueger paper, the treated group is larger than the control, but mine is much closer:
  * 331 NJ and 79 PA
  * 94 VA and 79 NC
...
### Selection Bias Problem
Because of the large number of missing counties in the employment level data, selection bias may influence the results. Counties that do not have the resources to collect employment level data may be more rural with a lower average income and may be more exposed to a minimum wage hike.
...
- Employment level data for food and beverage retailers (industry code: 445)
    * 2019 Q1 
    * 2021 Q4 
    * 2022 Q1
...
- Number of people with university degrees
    * US Census American Community Survey on Educational Attainment
    * 2020
- All Other Demographics
    * Calculated from County Population by Characteristics Dataset
    * 2019 
    * 2021
...
```{r Employmenttable, echo=FALSE, message=FALSE, warning=FALSE}


kable(
  tab_emp,
  col.names = c("Subset", "N", "Mean", "StdDev", "Min", "Max"),
  digits = 0,
  caption = "Descriptive Statistics: Food Service Employment Level ($TotalVA = 133$ $TotalNC = 100$)"
)
...
```{r 2019, echo=FALSE, message=FALSE, warning=FALSE}

# Create table for year 2019
kable(tab_Edu, 
      col.names = c("Measure", "Mean", "StdDev", "Min", "Max"), 
      digits = 2,
      caption = "Descriptive Statistics: Education Data 2020 ($All 233 Counties$)")

# Create table for year 2019
kable(tab_2019, 
      col.names = c("Measure", "Mean", "StdDev", "Min", "Max"), 
      digits = 2,
      caption = "Descriptive Statistics: Data for Year 2019 ($All 233 Counties$)")

```

## Data

```{r 2021, echo=FALSE, message=FALSE, warning=FALSE}


# Create table for year 2021
kable(tab_2021, 
      col.names = c("Measure", "Mean", "StdDev", "Min", "Max"), 
      digits = 2,
      caption = "Descriptive Statistics: Data for Year 2021 ($All 233 Counties$)")
```
...
- Time
  ...
  * 1 for Final
- Treatment
  * 1 for the VA after the treatment
  * 0 for all else
...
- According to the Lasso threshold Percent Hisp/Latino should be a good predictor of employment, but after dropping the other weak variables from the model in Table 6, it actually gets significance shifted away from it

- Of the variables listed as highly significant (p < .001) in table 5, Percent Hisp/Latino is the only one still in our model 

- This implies that it is only good at predicting employment insofar as it is good at differentiating between North Carolina and Virginia

- We should expect to see a large difference in concentrations of Hispanic populations in the two states

---

## Variable Selection
### Percent Hisp/Latino and Population Density are Good Classifiers 

- In Table 7 I've run a logistic regression on Percent Hisp/Latino and Population Density compared to all the other demographics 

- Hisp and Pop got an error rate of 27% and all the others got an error rate of 38%
...
## Variable Selection
### Variables in Question: Gender

- Gender was not found to be significant in the initial regression
- Gender was shown to be a good classifier
- Gender was below the lambda threshold in the Lasso regression
- All three results imply Gender should be dropped in the final model

## Variable Selection
### Variables in Question: Percent Hisp/Latino

- Percent Hisp/Latino was not found to be significant in the initial regression (p<.1)
- Percent Hisp/Latino was shown to be good a classifier
- Percent Hisp/Latino is directly on the lambda threshold
- The first two items alone advise dropping it from the model and since its not definitively selected by the lasso as a good predictor it can be safely dropped

## Variable Selection
### Variables in Question: Population Density

- Population Density was found to be significant in the initial regression
- Population Density was shown to be a good classifier
- Population Density was below the lambda threshold in the Lasso regression
- These results imply that Population Density is only initially significant because it is proxying the difference of employment level between the two states
